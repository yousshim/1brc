...
-- Show failing stations with details
SELECT 
    station_name,
    actual_min,
    expected_min,
    min_diff as min_diff,
    actual_max,
    expected_max,
...
FROM (
    ...
    FROM (SELECT station_name,
          ...
          FROM READ_CSV('actual.txt',
                        header=false,
                        columns={'station_name': 'VARCHAR', 'min_val': 'DECIMAL(8, 1)', 'max_val': 'DECIMAL(8, 1)', 'mean_val': 'DECIMAL(8, 1)'},
                        delim='/', parallel=true)) a
    JOIN (SELECT station_name,
                 MIN(measurement) as min_measurement,
                 MAX(measurement) as max_measurement,
                 CAST(AVG(measurement) AS DECIMAL(8, 1)) as mean_measurement
          FROM READ_CSV('sample.txt',
                        ...
                        columns={'station_name': 'VARCHAR', 'measurement': 'DECIMAL(8, 1)'},
                        delim=';', parallel=true)
          GROUP BY station_name) g ON a.station_name = g.station_name
)
WHERE min_diff > 0.1 OR max_diff > 0.1 OR mean_diff > 0.1
